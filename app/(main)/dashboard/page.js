...
const dashboardData = {
  ...
  attentionItems: [
    {
      id: 1,
      text: "Laptop #78A9 an. Budi Susanto sudah melewati batas waktu pinjam.",
      type: "warning",
    },
    {
      id: 2,
      text: "Garansi Proyektor #PJK04 akan habis dalam 15 hari.",
      type: "warning",
    },
    {
      id: 3,
      text: "Stok 'Tinta Printer Hitam' di bawah ambang batas minimum.",
      type: "danger",
    },
    {
      id: 4,
      text: "Perawatan rutin untuk Mobil #B1234XYZ dijadwalkan besok.",
      type: "info",
    },
  ],
...
function TotalAssetCard({ count }) {
  return (
    <div className="bg-white p-6 rounded-xl shadow-lg h-full flex flex-col justify-between">
      ...
      <a
        href="/inventory"
        className="text-blue-500 hover:text-blue-700 font-semibold mt-4 inline-flex items-center"
      >
...
function DataShortcutCard({ title, value, unit, icon: Icon, href, iconColor }) {
  return (
    <a
      href={href}
      className="bg-white p-6 rounded-xl shadow-lg flex items-center space-x-4 hover:bg-gray-50 transition transform hover:scale-105"
    >
      <div
