...
export default function BackupSettings({ settings, onSettingChange, isDarkMode }) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div>
          <label className={labelClass}>Frekuensi Backup</label>
          <select value={settings.backupFrequency} onChange={(e) => onSettingChange('backup', 'backupFrequency', e.target.value)} className={inputClass}>
        ...
        <div>
          <label className={labelClass}>Retention (Hari)</label>
          <input type="number" value={settings.retentionDays} onChange={(e) => onSettingChange('backup', 'retentionDays', parseInt(e.target.value))} className={inputClass} />
        </div>
      </div>
      
      <div className="space-y-4">
         {Object.entries(backupToggles).map(([key, label]) => (
            <label key={key} className={`flex items-center justify-between p-4 border rounded-lg cursor-pointer transition-colors ${isDarkMode ? 'border-gray-700 hover:bg-gray-700' : 'border-gray-200 hover:bg-gray-50'}`}>
                ...
                <div className="relative inline-flex items-center cursor-pointer">
                    <input
                        ...
                        onChange={(e) => onSettingChange('backup', key, e.target.checked)}
