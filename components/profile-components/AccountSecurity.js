// /components/profile/AccountSecurity.js
"use client";
...
export default function AccountSecurity({ user }) {
  return (
    <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-200">
      <h3 className="text-lg font-bold text-gray-800 border-b pb-3 mb-4">Keamanan Akun</h3>
      <div className="space-y-5">
        ...
        <div className="flex items-center justify-between">
          ...
          <span className={`text-sm px-2 py-0.5 rounded-full ${user.twoFactorEnabled ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-700'}`}>
        ...
        <div className="flex items-start space-x-3 pt-4 border-t">
          ...
          <div>
            ...
            <p className="text-xs text-gray-500">{user.lastLogin.timestamp} dari IP {user.lastLogin.ip}</p>
          </div>
        </div>
      </div>
    </div>
