...
export default function TrackingAnalyticsView({ trackingData }) {
  ...
  const onGoingItems = trackingData.filter(
    (item) => !["terkirim", "di lokasi"].includes(item.status.toLowerCase())
  ).length;
  const completedItems = trackingData.filter((item) =>
    ["terkirim", "di lokasi"].includes(item.status.toLowerCase())
  ).length;
  const averageProgress =
    totalItems > 0
      ? Math.round(
          trackingData.reduce((sum, item) => sum + item.progress, 0) /
            totalItems
        )
      : 0;
